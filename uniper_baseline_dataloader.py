...
def readMainActivitySheet(wkbook,sheetindex):
    try:
        ...
        ## close the excel application - call close_excel
        close_excel(wkbook)
...
def expandDates(result_data):
    try:
      ...
      #  db_conn = dbu.getConn()  -----------#### uncomment this line
...
def insertActivity():
    try:
        #db_conn = dbu.getConn() ----------###  Uncomment this line in production
        ...
        for i in range(0, totalRec):
            ...
            print(execSQL,execData,file=f)
                # dbu.executeQueryWithData(db_conn, execSQL, execData)-----------#### uncomment this line
...
# This function is called from readMainActivitySheet()
def close_excel(wb):
    wb.ActiveWorkBook.Close() # closes the active workbook
