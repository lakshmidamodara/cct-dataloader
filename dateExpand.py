    ...
    try:
        ...
        for i in range(0,totalRecords):
            for j in range (1,2):
                ...
                # Now execute the query to insert data into activities table
                #dbu.executeQueryWithData(db_conn,  execData)
...
def readHolidaysExcel(wkbook,sheetindex):
    try:
        ...
        for curr_row in range(2, m_row, 1):
            ...
            if data.value != None:
                dtDate = datetime.strptime(data.value, '%B %d,%Y')
                holiday_data.append(dtDate.date())
        #print(holiday_data)
    except (Exception) as error:
